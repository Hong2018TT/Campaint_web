...
<div class="shadow-lg rounded-md bg-white">
    ...
    <div class="mt-5 flow-root pb-2 px-3">
        ...
        <form id="form-color" name="from-color" calss="max-w-md mx-auto" action="{{ route('update_color', $color->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 sm:gap-2 md:gap-4 mt-3">
                ...
                <div class="box-form">
                    ...
                    <div class="grid grid-cols-1 focus-within:relative">
                        <select id="color_family" name="color_family" autocomplete="color_family" aria-label="category" class="form-select" required>
                            <option value="" hidden selected>Select color family</option>
                            @foreach ($colorfamilys as $colorfamily)
                                <option value="{{ $colorfamily->name }}" {{ $colorfamily->name == $colorfamily->name ? 'selected' : '' }}>
                                    {{ $colorfamily->name }}
                                </option>
                            @endforeach
                        </select>
